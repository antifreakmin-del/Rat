-- [[ DARK HUB // MOBILE v5 - ULTRA STABLE ]]
...
-- Очистка старых копий UI и эффектов
pcall(function()
...
local hlS = Instance.new("UIStroke")
hlS.Color = Theme.Accent
hlS.Thickness = 1.5
hlS.Parent = hintLabel
...
local function addDropdown(parent, text, options, def, cb)
    ...
    vb.Size = UDim2.new(0, 82, 0, 22)
    vb.Position = UDim2.new(1, -88, 0, 5)
...
table.insert(aimElements.normalGroup, addSlider(aimPage, "Плавность", 0.05, 1, State.aimSharpness, true, function(v)
    State.aimSharpness = v
end))

table.insert(aimElements.normalGroup, addSlider(aimPage, "Макс. дистанция", 50, 1500, State.aimMaxDistance, false, function(v)
...
table.insert(aimElements.autoGroup, addSection(aimPage, "AUTO-SHOOT ПОЗИЦИЯ"))
...
table.insert(aimElements.autoGroup, addSection(aimPage, "AUTO-SHOOT ПАРАМЕТРЫ"))

table.insert(aimElements.autoGroup, addToggle(aimPage, "Обзор 360°", State.auto360, function(v)
    State.auto360 = v
end))

table.insert(aimElements.autoGroup, addDropdown(aimPage, "Вид камеры", {"1st", "3rd"}, State.autoView, function(v)
    State.autoView = v
    if v == "1st" then
...
table.insert(aimElements.autoGroup, addToggle(aimPage, "Silent (без поворота)", State.autoSilent, function(v)
    State.autoSilent = v
end))
...
table.insert(aimElements.autoGroup, addSlider(aimPage, "Скорость доводки", 0.05, 1, State.aimSharpness, true, function(v)
    State.aimSharpness = v
end))

-- По умолчанию открыт обычный режим
...
local function applyHighlight(model, isPlayer)
    ...
    local stale = model:FindFirstChild("PlayerHighlight"); if stale then pcall(function() stale:Destroy() end) end
    local staleBb = model:FindFirstChild("NameESP"); if staleBb then pcall(function() staleBb:Destroy() end) end
    ...
    bb.Adornee = model:FindFirstChild("Head") or model.PrimaryPart or model:FindFirstChild("HumanoidRootPart")
    bb.Parent = model
...
Players.PlayerRemoving:Connect(function(p)
    if p.Character then
        if espHighlights[p.Character] then pcall(function() espHighlights[p.Character]:Destroy() end) end
        espHighlights[p.Character] = nil
        if espBillboards[p.Character] then pcall(function() espBillboards[p.Character]:Destroy() end) end
        ...
        if target then
            ...
            if State.autoSilent then
                ...
                if autoShootCooldown <= 0 then
                    pressAutoButton()
                    autoShootCooldown = State.autoDelay + 0.1
                end
                ...
                if head then
                    ...
                    if d < 12 and autoShootCooldown <= 0 then
                        pressAutoButton()
                        autoShootCooldown = State.autoDelay + 0.1
                    end
                end
