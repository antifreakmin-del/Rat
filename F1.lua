    ...
    gui.InputEnded:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
            if tick() - t < 0.5 then
                ...
                if not ok then warn("[bindTap]", err) end
            end
        end
    end)
end
...
local function addColorPicker(parent, text, def, cb)
    ...
    row.Size = UDim2.new(0, 170, 0, 26)
    row.Position = UDim2.new(1, -178, 0.5, -13)
...
nafLayout.Padding = UDim.new(0, 6)
...
aafLayout.Padding = UDim.new(0, 6)
aafLayout.Parent = autoAimFrame

local aafSize = Instance.new("AutomaticSize")
aafSize.Parent = autoAimFrame

-- Кнопка "Авто-выстрел" — на верхнем уровне, вне обоих фреймов
addToggle(aimPage, "⚡ Авто-выстрел (режим)", State.autoShoot, function(v)
    State.autoShoot = v
...
addToggle(autoAimFrame, "Silent (визуально не поворачиваюсь)", State.autoSilent, function(v)
    State.autoSilent = v
end)
...
-- Отдельный обработчик тапов для кнопки (на случай если bindTap глючит)
local lastCircleTap = 0
...
CircleBtn.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch
    or input.UserInputType == Enum.UserInputType.MouseButton1 then
        if tick() - lastCircleTap < 0.4 then
            ...
            if hum and hum.Health > 0 and part then
                ...
                if d3 <= State.aimMaxDistance then
                    ...
                    if onScreen then
                        ...
                        if (useFullCircle or d2 <= bestDist) and isVisible(part) then
                            if d2 < bestDist then
                                bestDist = d2
                                best = part
                            end
...
print("[DarkHub Mobile v2.1] Загружено")
