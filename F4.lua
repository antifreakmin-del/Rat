...
local allPlayers = {}       -- [Player] = { name = string, highlighted = bool }

-- ============================================================
-- УНИВЕРСАЛЬНЫЙ TAP/CLICK (чинит кнопки на телефоне)
-- ============================================================
...
local fovBtn = createAimSubBtn("FOV: 110", function()
    State.aimFov = (State.aimFov == 110) and 160 or 110
    fovBtn.Text = "FOV: " .. tostring(State.aimFov)
    fovCircle.Size = UDim2.new(0, State.aimFov * 2, 0, State.aimFov * 2)
end)

local sharpBtn = createAimSubBtn("Резкость: Ср", function()
    if State.aimSharpness == 0.25 then
        State.aimSharpness = 0.5
        sharpBtn.Text = "Резкость: Выс"
    elseif State.aimSharpness == 0.5 then
        State.aimSharpness = 0.1
        sharpBtn.Text = "Резкость: Мягк"
    else
        State.aimSharpness = 0.25
...
-- ============================================================
-- СВИТЧИ (через bindTap)
-- ============================================================
local function bindSwitch(switchBtn, knob, key, onStateChange)
    bindTap(switchBtn, function()
        State[key] = not State[key]
        local newState = State[key]
        ...
        if onStateChange then onStateChange(newState) end
    end)
end

bindSwitch(espSwitch, espKnob, "espEnabled", function(val)
    -- Применяем/снимаем подсветку для ВСЕХ игроков
...
bindSwitch(aimSwitch, aimKnob, "aimEnabled", function(val)
...
-- ============================================================
-- ESP: ПОДСВЕТКА ВСЕХ ИГРОКОВ (работает после респавна)
-- ============================================================

-- Создать / пересоздать highlight для игрока
local function refreshHighlight(player)
    if player == LocalPlayer then return end
    local char = player.Character
    if not char then return end

    -- Ждём Humanoid, чтобы highlight не висел на пустой модели
    ...
    local stale = char:FindFirstChild("PlayerHighlight")
...
-- Регистрация игрока: запоминаем ник + цепляем подсветку
local function registerPlayer(player)
    ...
    player.CharacterAdded:Connect(function()
        -- Небольшая задержка, чтобы модель прогрузилась
        task.wait(0.2)
...
-- Если ESP включён, но игрок появился — Highlight уже создан с Enabled = State.espEnabled
-- Ничего дополнительно делать не надо, всё синхронно.
...
-- Целевая точка — не только голова, но и HumanoidRootPart как фоллбэк
        ...
        if player ~= LocalPlayer and player.Character then
            ...
            if hum and hum.Health > 0 and targetPart then
                ...
                if onScreen then
                    ...
                    if dist <= minDistance and isVisible(targetPart) then
                        minDistance = dist
